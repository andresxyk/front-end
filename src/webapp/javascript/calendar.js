...
// CALENDAR COLORS
topBackground    = "#f7f7f7";         // BG COLOR OF THE TOP FRAME
bottomBackground = "#f7f7f7";         // BG COLOR OF THE BOTTOM FRAME
tableBGColor     = "black";         // BG COLOR OF THE BOTTOM FRAME'S TABLE
cellColor        = "#E7EDF5";     // TABLE CELL BG COLOR OF THE DATE CELLS IN THE BOTTOM FRAME
//headingCellColor = "#FF9933";         // TABLE CELL BG COLOR OF THE WEEKDAY ABBREVIATIONS
headingCellColor = "#A9C6EA";         // TABLE CELL BG COLOR OF THE WEEKDAY ABBREVIATIONS
...
hoverColor       = "#990033";       // TEXT COLOR OF A LINK WHEN YOU HOVER OVER IT
...
/**
* Funciones auxilares encargadas de formatear el
* campo de fecha que recive como parametro el calendario
*/
function getFechaCalendar(fecha){
	var arrFecha = fecha.split(/-/);
    var dia = parseInt(arrFecha[0]);
    var mes = getNumMesCalendar(arrFecha[1])-1;
...
*/
function getNumMesCalendar(mes){
	var intMes = 0;
    if(mes != null){
		mes = mes.toUpperCase();
    	if( mes == "ENE" ) { intMes = 1; }
...
function setDateField(dateField) {
	...
	if(dateField.value != ""){
		inDate = getFechaCalendar(dateField.value);
	}
	else{
		inDate = "";
	}
...
function showCalendar(dateField) {
    ...
    calDocFrameset = 
        ...
        "  <FRAME NAME='topCalFrame' SRC='javascript:parent.opener.calDocTop' SCROLLING='no' RESIZABLE='no'>\n" +
        "  <FRAME NAME='bottomCalFrame' SRC='javascript:parent.opener.calDocBottom' SCROLLING='no' RESIZABLE='no'>\n" +
        "</FRAMESET>\n";
	var winPrefs = "RESIZABLE='no'";
...
function buildTopCalFrame() {
	//Se agrega el anio actual del sistema
	var today = new Date();
    var anio = today.getYear()+1;
    ...
    var calDoc =
        ...
        "<CENTER>" +
        getMonthSelect() +
        "<SELECT NAME='year' onChange='parent.opener.setYear()'>"
        for(var iyear=1890; iyear<=anio; iyear++){
			calDoc += "<option "
			if(iyear==calDate.getFullYear()){ calDoc += " selected "}
			calDoc += "value='" + iyear + "'>" + iyear
        }
        calDoc += "</SELECT>"

//     Sustituimos la l�nea de abajo por un <SELECT>
