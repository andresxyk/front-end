...
//							objFormato = new FormatoFacturaGlobalTextoConDescuentoImpl();												
					...
					if (strCorreoElectronico.trim().toString().length() > 2) {
						...
						objMailDAO.sendEmail("Factura Electrónica, OLAB Diagnósticos Médicos, gracias por su preferencia", strCorreoElectronico, strPDFRead + "FacturacionElectronica_" +  objFacturaBean.getSseriofoliocompleto() + ".pdf", strXMLRead + "FacturacionElectronica_" +  objFacturaBean.getSseriofoliocompleto() + ".xml");
					}
		...
		try {			
			   ...
			   objFacturaBean=this.crearFacturaElectronica(objNotadeCreditoDAO.generarNota(strfacturasElegidas,cconvenio,strdescripcionnota,dmontonotacredito));
				...
				if ("bibiana.yanez@olab.com.m".trim().toString().length() > 2) {
					...
					String strPDFRead = ConfiguracionProperties.getPropiedad("reporte.ruta.pdflocalread");
					String strXMLRead = ConfiguracionProperties.getPropiedad("reporte.ruta.xmllocalread");
					objMailDAO.sendEmail("Factura Electrónica, OLAB Diagnósticos Médicos, gracias por su preferencia", "bibiana.yanez@olab.com.m", strPDFRead + "FacturacionElectronica_" +  objFacturaBean.getSseriofoliocompleto() + ".pdf", strXMLRead + "FacturacionElectronica_" +  objFacturaBean.getSseriofoliocompleto() + ".xml");
				}
