...
public class FormatoFacturaSinDescuentoImpl implements FormatoFactura {

	private static Log iObjLog = LogFactory.getLog(FormatoFacturaSinDescuentoImpl.class);

	private int cMarca = 1;
	
	public FormatoFacturaSinDescuentoImpl(int cMarca) {
		this.cMarca = cMarca;
	}
	...
	public FacturaElectronicaBean crearFacturaFormato(FacturaElectronicaBean objFacturaBean) throws Exception {
		...
		try {					
			...
			for (int inti = 0; inti < objOrdenBean.sizeExamenes(); inti++ ) {
				...
				if (objOrdenBean.getMpagapaciente() > 0.0) {
					objBody.setDblImporte(objBody.getDblImporte() + (objOrdenExamenBean.getMpagopaciente()) / 1.16);
				} else {
					objBody.setDblImporte(objBody.getDblImporte() + (objOrdenExamenBean.getMfacturaempresa()) / 1.16);
				}									
