...
public class FormatoFacturaGlobalTextoConDescuentoImpl implements FormatoFactura {

	private static Log iObjLog = LogFactory.getLog(FormatoFacturaGlobalTextoConDescuentoImpl.class);

	private int cMarca = 1;
	
	public FormatoFacturaGlobalTextoConDescuentoImpl(int cMarca) {
		this.cMarca = cMarca;
	}
	...
	public FacturaElectronicaBean crearFacturaFormato(FacturaElectronicaBean objFacturaBean) throws Exception {
		...
		try {					
			...
			Integer objKordenSucursalLong = null;
			List lstOrdenFacturar = objBuscarOrdenDAO.getOrdenesFacConvenio(objFacturaBean.getCconvenio());
			for (int intz=0;intz<lstOrdenFacturar.size();intz++) {
				objKordenSucursalLong = (Integer)lstOrdenFacturar.get(intz);
				OrdenBean objOrdenBean = objBuscarOrdenDAO.buscarOrdenExamen(objKordenSucursalLong.intValue(),true);			
				...
				for (int inti = 0; inti < objOrdenBean.sizeExamenes(); inti++ ) {
							...
							if (objOrdenBean.getMpagapaciente() > 0.0) {
								objBody.setDblImporte(objBody.getDblImporte() + (objOrdenExamenBean.getMpagopaciente() + dblSumaDescuento) / 1.16);
							} else {
								objBody.setDblImporte(objBody.getDblImporte() + (objOrdenExamenBean.getMfacturaempresa() + dblSumaDescuento) / 1.16);
							}					
			...
			objBody = new BodyFacturaElectronicaBean();
			objBody.setDblImporte(objFacturaBean.getMsubtotalsuma());
			objBody.setDblValorUnitario(objFacturaBean.getMsubtotalsuma());
			objBody.setIntCantidad(1);
			objBody.setStrCodigo("");
			objBody.setStrDescripcion("ESTUDIOS REALIZADOS EN EL MES DE MARZO SEGUN RELACION ADJUNTA CENTRO MEDICO ISSEMYM");
			objBody.setStrUnidad("unidad");
